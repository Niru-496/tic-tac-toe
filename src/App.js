...
class App extends React.Component {
	...
	computerTurn = () => {
		...
		if (isAvailable) {
			setTimeout(() => {
				var randomItem =
					notBlueOrRed[
						Math.floor(Math.random() * notBlueOrRed.length)
					];
	...
	checkForWinner = () => {
		...
		combinationCollection.map((data) => {
			if (
				this.refs["block" + data[0]].className &&
				this.refs["block" + data[1]].className &&
				this.refs["block" + data[2]].className &&
				this.refs["block" + data[0]].className.toLowerCase() ===
					this.refs["block" + data[1]].className.toLowerCase() &&
				this.refs["block" + data[1]].className.toLowerCase() ===
					this.refs["block" + data[2]].className.toLowerCase() &&
				this.refs["block" + data[2]].className.toLowerCase() ===
					this.refs["block" + data[0]].className.toLowerCase()
			) 
				{
			...
			<div className="App">
				...
				<div class="scores p1">
					<p class="player1">
						...
						<span class="x"></span>)
						<span class="score">{this.state.playerWin}</span>
					</p>
					<p class="ties">
						Tie<span class="score">{this.state.tieGames}</span>
					</p>
					<p class="player2">
						<span class="p1">Computer</span>
						<span class="p2">Player 2</span> (
						<span class="o"></span>)
						<span class="score">{this.state.computerWin}</span>
					</p>
				</div>
			</div>
		);
